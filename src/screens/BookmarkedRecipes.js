...
const BookmarkedRecipes = () => {
  ...
  useEffect(() => {
    const fetchBookmarked = async () => {
      try {
        ...
        const res = await fetch(`${API_BASE_URL}/api/users/recipesBookmark?uid=${uid}`);
        const data = await res.json();
        setBookmarkedRecipes(data);
      } catch (err) {
        console.error("즐겨찾기 레시피 불러오기 실패:", err);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>즐겨찾기한 레시피</Text>
      {loading ? (
